...
class Stroke {
  ...
  func emit_(output: UnsafeMutablePointer<UInt8>,
    ...
    let target1 = output
    target1.assignFrom(vertexPtr, count: vertexSize)
    ...
    let target2 = output.advancedBy(vertexSize)
    vertex.pX = vertex.pX - (flipX * width)
    vertex.pY = vertex.pY - (flipY * width)
    vertex.nX = -flipX
    vertex.nY = -flipY
    target2.assignFrom(vertexPtr, count: vertexSize)
    print("Emitting odd vertex: \(vertex)")
  }
    
  func tesselate_(output: UnsafeMutablePointer<UInt8>, totalVertexCount: Int, lengthOffset : Float) {
