...
echo "$(tput setaf 2)"
echo "###################################################################"
echo "# Preparing to build Google gRPC"
...
GRPC_VERSION=0_9_1
GRPC_RELEASE_URL=https://github.com/google/grpc/grpc/archive/release-${GRPC_VERSION}.tar.gz
GRPC_RELEASE_DIRNAME=grpc-${GRPC_VERSION}
...
echo "GRPC_GIT_URL ........... ${GRPC_GIT_URL}"
echo "GRPC_GIT_DIRNAME ....... ${GRPC_GIT_DIRNAME}"
echo "GRPC_VERSION ........... ${GRPC_VERSION}"
echo "GRPC_RELEASE_URL ....... ${GRPC_RELEASE_URL}"
echo "GRPC_RELEASE_DIRNAME ... ${GRPC_RELEASE_DIRNAME}"
...
(
    ...
    then
        ...
        then
            rm -rf "${GRPC_RELEASE_DIRNAME}"
        fi
        curl --location ${GRPC_RELEASE_URL} --output ${GRPC_RELEASE_DIRNAME}.tar.gz
        tar xvf ${GRPC_RELEASE_DIRNAME}.tar.gz
        mv "${GRPC_RELEASE_DIRNAME}" "${GRPC_SRC_DIR}"
        rm ${GRPC_RELEASE_DIRNAME}.tar.gz

        # Remove the version of Google Test included with the release.
        # We will replace it with version 1.7.0 in a later step.
        if [ -d "${GRPC_SRC_DIR}/gtest" ]
        then
            rm -r "${GRPC_SRC_DIR}/gtest"
        fi
    fi
)
...
then
    ...
    if [ -d "${PREFIX}-${GRPC_VERSION}" ]
    then
        rm -rf "${PREFIX}-${GRPC_VERSION}"
    fi
    mv "${PREFIX}" "${PREFIX}-${GRPC_VERSION}"
fi
